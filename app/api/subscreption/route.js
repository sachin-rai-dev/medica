...
export async function GET(req) {
    ...
    try {
        ...
        if (find_dummy_user_data.subcreptionID == undefined) {
            return NextResponse.json({doctorId: find_dummy_user_data.userid})
        }else{
            const subscription = await stripe.subscriptions.retrieve(find_dummy_user_data.subcreptionID)
            
            return NextResponse.json({ data: subscription, plan: find_dummy_user_data.subcreption, expiredate: find_dummy_user_data.expiredate, date: find_dummy_user_data.date, doctorId: find_dummy_user_data.userid })
        }
